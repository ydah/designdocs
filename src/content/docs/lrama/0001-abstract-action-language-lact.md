...
Lrama のセマンティックアクションを宿主言語から分離するため、型付きの小さな抽象アクション言語 **Lact** を導入する提案である。既存の C アクションは既定動作として維持し、Lact を選んだ文法だけが新しい変換パイプラインを使う。
...
現在の Lrama はアクション本文を C として構文解析せず、`$$` や `$n` などの参照だけを文字列置換している。この既存の抽象化点を利用すれば、従来経路を保ったまま Lact の解析・検査・コード生成を追加できる。制御構造や評価順序を共通 IR までに解決し、バックエンドをレンダリングへ限定することで、対象言語を増やしたときの実装差分も抑えられる。
...
| (B) **アクションの多言語化** | ユーザが書いた `{ $$ = $1 + $3; }` を対象言語へ変換する | 高（本質的に未解決） |

本設計書は **(B) を主題**とし、(A) は (B) を実証するために必要な範囲で扱う。

### 1.1 想定ユースケース

1. **同一文法からの複数言語パーサ生成** — 1 つの `.y` から C 版と JS 版を生成し、ブラウザ／Node 上で同じ構文解析を行う
...
1. **アクション本文は一切パースされていない。** 単なる文字列置換である。したがって「抽象言語のパーサを後段に足す」形で、既存経路を壊さずに拡張できる。
2. **抽象化点はすでに存在する。** `reference_to_c` という 1 メソッドが C 依存の全てを担っている。ここをバックエンドに委譲すれば、`$n` → スタックオフセットの計算（言語非依存）と、その**レンダリング**（言語依存）を分離できる。
3. **型情報の唯一の源はタグ (`%union` のメンバ名) である。** `%type <val> expr` の `val` が `$$` / `$n` の型を決める。抽象言語の型システムはこれを基盤にすべきである。
4. **パラメータ化規則は `Reference` オブジェクトを直接書き換える。** `Grammar::Parameterized::Rhs#resolve_user_code` は `s_value` ではなく `references` の `name` を書き換えることで `$x` → `$expr` の解決を行う。**Lact フロントエンドも同じ `Reference` オブジェクトを共有しなければ、`%rule` / `%inline` が壊れる**（§8.4 で対処）。
...
**設計の要点は「バックエンドを可能な限り薄くする」こと**である。制御構造の平坦化（`if` 式 → 一時変数 + `if` 文）や一時変数の命名まで Lowering で済ませておけば、バックエンドは IR ノードごとに 1 行の文字列を返すだけの単純な訪問者になる。これが G4（バックエンド追加の低コスト化）を担保する。
...
> **実装上の利点:** `Lexer#lex_c_code` は中括弧の対応数と文字列リテラル `"..."` / `'...'` しか見ていないため、Lact のソースもそのまま「本文文字列」として切り出せる。**字句解析器の変更はほぼ不要**であり、この点が本設計の実装コストを大きく下げている。
>
> 要確認のエッジケース: Lact のコメント記法（`//` と `/* */` を採用し、C と揃える）、`%` 始まりトークン (`%raw`) がブロック内に現れても `lex_c_code` は素通しにすること。
...
一時変数名は `yylact_0`, `yylact_1`, ... とする（`yy` 接頭辞は Bison 系で予約されており、ユーザコードとの衝突リスクが最小）。
...
> **Lact パーサを Racc で生成しない理由:** Lrama 自身は `parser.y` を Racc でビルドしているが、Lact のパーサは (1) 文法が小さい (2) エラーメッセージの品質が重要 (3) ビルド依存を増やしたくない、という理由から**手書き**を推奨する。式部分は Pratt パーシングで 100 行程度に収まる。
...
これを満たさないと、`%rule` / `%inline` を使った文法で Lact アクションの参照解決が静かに壊れる。**Phase 1 のテストで必ず検証すること。**
...
これはアクションの値として「C の演算子トークンそのもの」を持たせ、`$1 $2 $3` というトークン連結で C 式を組み立てる技法である。Lact は式の言語であってトークン生成の言語ではないため、原理的に対応できない。Lact 版では `%extern` で `add`/`sub` を宣言し、規則を分けて書く必要がある。**この種の「C のテキストとしての性質」に依存した技法が既存文法にどれだけあるかを、移行前に調査すべきである。**
...
| 案 | 内容 | 評価 |
| --- | --- | --- |
| A. 言語別にアクションを併記 | `%c { ... } %js { ... }` を全アクションに書く | 実装は最小だが DRY でなく、規模が増えると破綻する。ただし**部分採用**した（`%raw`） |
