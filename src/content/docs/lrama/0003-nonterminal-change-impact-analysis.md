...
description: 非終端記号の変更が記号・規則・状態・意味アクションへ伝播する範囲を静的に解析し、CLIから出力する機能の設計提案
status: proposed
...
| 基準リビジョン | [`1e9294d`](https://github.com/ruby/lrama/commit/1e9294d017762b18327742ac9034411f7f0f1c7f)（2026-06-22 UTCの `master`） |
| コード上のバージョン | Lrama 0.8.0（基準リビジョンの `lib/lrama/version.rb`） |
| 調査時点 | 2026-08-20 |
| 実装状態 | 未実装の設計提案 |
| 文書の目的 | 非終端記号の変更影響を静的に追跡する機能の要件、アルゴリズム、CLI、出力、段階導入を定義する |
| 検証範囲 | 基準リビジョンのAPIと処理フローに基づく静的設計。性能目標と偽陰性なしの性質は実装後に検証する |

Lrama に **impact analysis** を追加する。非終端記号の規則変更が、記号・規則・意味アクション・LR 状態へ及ぼしうる範囲を列挙する機能である。変更前の見積り、レビュー、conflict の原因追跡、CI での差分確認に使えるよう、伝播経路と理由を人間向け・機械向けの両形式で出力する。

## 提案

- `Lrama::Impact` を独立した解析コンポーネントとして追加し、正規化済み文法から記号依存グラフを構築する。
- `--impact=SYMBOLS` を診断専用モードとして提供し、指定時はパーサ生成物を書き出さない。
- `symbol`、`rule`、`action`、`state` の4レベルで影響を報告し、各項目に伝播方向、経路、理由を付ける。
- 既存記号を使った変更について偽陰性を出さない保守的近似を受け入れ条件とし、偽陽性は深さ制限と理由分類で制御する。
- text、JSON、DOT を出力し、差分モードとエディタ連携を段階的に追加する。
- `--impact` 未指定時の生成結果と実行経路は変えない。

## 判断理由

既存の `--report`、`--trace`、`--diagram` は文法全体の状態を示すが、「ある非終端記号を変えるとどこまで影響しうるか」には直接答えない。大規模文法では、依存先、先読み集合、LR 状態、conflict が離れた場所で変化する。変更箇所だけを読んで影響範囲を説明するのは難しい。

正規化済み規則から依存グラフを作り、`nullable` / `FIRST` / `FOLLOW` の伝播方向と LR 状態の関係を分けて追跡する。これにより、影響理由を保ったまま保守的に列挙できる。記号・規則・アクションまでなら `States#compute` を避けられるため、対話的な利用にも向く。

## 影響範囲

新規の `Lrama::Impact` 名前空間、CLI オプション、診断出力を追加する。状態レベル解析では既存の Reads / Includes / Lookback 関係を参照し、`%inline` の展開元を追跡するために resolver へ小さな記録機能を追加する。通常の C コード生成、既存レポート、`--impact` を指定しない実行は変更対象外とする。

## 検討した選択肢と採用しなかった理由

| 選択肢 | 判断 | 理由 |
| --- | --- | --- |
| 文法全体の既存レポートだけを使う | 不採用 | 起点となる記号からの伝播経路と理由を復元できず、レビューや対話利用に向かない |
| 変更前後の生成テーブル差分だけを比較する | 初版では不採用 | 実際の変更後にしか使えず、規則番号の再採番によるノイズが大きい |
| conflict の増減を事前に厳密予測する | 不採用 | 変更内容が未確定の段階では断定できず、誤った保証になる |
| 状態計算を常に実行して精度を上げる | 不採用 | 記号・規則・アクションの反復解析まで遅くなるため、状態レベルを選んだ場合だけ実行する |
| 保守的な依存解析を行い、差分モードで精度を補う | 採用 | 変更前の見積りとCIでの実差分を同じデータモデルで扱える |

## この文書の読み方

| 読者 | 最初に確認する節 |
| --- | --- |
| 提案をレビューする人 | 提案、判断理由、ゴール / 非ゴール、後方互換性とリスク、提案内容の確認事項 |
| CLI 利用者 | ユースケース、CLI 設計、出力仕様、差分モード |
| 実装する人 | アーキテクチャ、データモデル、アルゴリズム、実装計画、テスト計画 |
| 解析精度を確認する人 | 「影響」の定義、保守的近似の保証と限界、計算量まとめ |
...
- ある非終端記号の規則を 1 行変えたとき、どの規則・どの状態・どのアクションが影響を受けるのか、人間には追えない。
...
非終端記号 `A` を起点に、その規則変更が及ぼしうる範囲を静的に列挙する。具体的な提供物は冒頭の「提案」に示す。
...
| 用語 | 定義 |
|---|---|
| ターゲット記号 | 影響解析の起点として利用者が指定する非終端記号 |
...
上流と下流の両方を報告する。上流だけを解析する実装は多いが、下流の `FOLLOW` 変化を見落とすと、「なぜ関係ない場所で conflict が出たのか」を説明できない。
...
`Δ_nullable` を先に計算し、その結果を prefix 判定に使う。`nullable` が変わりうる記号を nullable とみなし、安全側に解析する。
...
影響解析は `Grammar#prepare` 後の正規化済み規則に対して行う。一方、利用者が編集するのは元の `.y` ソースである。そのため、両者を対応づける必要がある。
...
現行文法に存在する記号・規則については、次の変更で影響を受ける箇所をすべて出力に含める。対象は、`A` の規則の削除、RHS の記号順序の変更、既存記号を使った代替規則の追加である。上流・下流の両方向を推移閉包で辿り、`nullable` の変化を安全側に丸めて、この性質を満たす。
...
- 影響記号には「経路」と「変わりうる計算の種類」を併記する。理由を示さない一覧は採用しない。
...
2. 各規則について**安定署名**を計算する。
   ...
   ```
   `normalize` は midrule 記号名 (`$@1`, `@2`) を「親規則名 + RHS 内位置」に置き換える。連番はわずかな編集でずれる。そのまま比較すると、差分に多くのノイズが生じる。
