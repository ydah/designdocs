...
Lrama に、非終端記号のルール変更が記号・規則・意味アクション・LR 状態へ及ぼしうる範囲を列挙する **impact analysis** を追加する提案である。変更前の見積り、レビュー、conflict の原因追跡、CI での差分確認に使えるよう、伝播経路と理由を人間向け・機械向けの両形式で出力する。
...
既存の `--report`、`--trace`、`--diagram` は文法全体の状態を示すが、「ある非終端記号を変えるとどこまで影響しうるか」には直接答えない。大規模文法では、依存先、先読み集合、LR状態、conflict が離れた場所で変化するため、変更箇所だけを読んで影響範囲を説明するのは難しい。

正規化済み規則から依存グラフを作り、`nullable` / `FIRST` / `FOLLOW` の伝播方向とLR状態の関係を分けて追跡すれば、影響理由を保ったまま保守的に列挙できる。記号・規則・アクションまでなら `States#compute` を避けられるため、対話的な利用にも向く。
...
| 読者 | 最初に確認する節 |
| --- | --- |
| 提案をレビューする人 | 提案、判断理由、ゴール / 非ゴール、後方互換性とリスク、未決事項 |
...
現行の文法に既に存在する記号・規則に関しては、`A` の規則を「削除する」「RHS の記号の順序を変える」「既存の記号を使った代替規則を追加する」といった変更で影響を受ける箇所を、すべて出力に含める。上流・下流の両方向を推移閉包で辿り、`nullable` の変化を安全側に丸めることで、この性質を満たす設計とする。
